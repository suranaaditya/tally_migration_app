...
the closing snapshot. We caught this on GHRCACS purely because we had
...
by (name, tally_id).** GHRCACS contains two real ledgers both named
...
not a parser bug.** GHRCACS's trial balance is internally imbalanced
...
**Large fixtures live outside the repo.** The full 221 MB GHRCACS
...
(`sample_ghrcacs_masters_sample.xml` — first 500 ledgers + 13
