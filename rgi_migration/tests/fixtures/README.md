...
|---|---|---|---|
| `sample_cacspu_masters_sample.xml`      | **yes** | ~10 MB | Default XML fixture — 587-ledger curated sample from the full export |
| `sample_cacspu_opening_tb.xlsx`         | **yes** | 29 KB  | Reference opening-TB Excel with explicit Dr/Cr columns; used by the Excel parser and the XML↔Excel cross-check |
...
The full 221 MB `cacspu_masters.xml` is **not** stored in this directory
and is **not** version-controlled. By convention it lives under
`~/tally-exports/` on each developer's machine:

```
~/tally-exports/cacspu_masters.xml   (221 MB)
```
...
`sample_cacspu_masters_sample.xml` is produced by
...
1. Open Tally with the **CACSPU** company loaded.
...
3. Save the output to **any location outside this repo**. The convention
   is `~/tally-exports/cacspu_masters.xml` but anywhere works.
4. Set the env var pointing at the absolute path:
   ```
   export RGI_FULL_XML_PATH=~/tally-exports/cacspu_masters.xml
   ```
   (On Windows: `$env:RGI_FULL_XML_PATH = "$env:USERPROFILE\tally-exports\cacspu_masters.xml"`)
...
6. Run the full-file regression suite:
   ```
   RGI_FULL_XML_PATH=~/tally-exports/cacspu_masters.xml \
...
cp ~/tally-exports/cacspu_masters.xml \
   rgi_migration/tests/fixtures/sample_cacspu_masters.xml
python scripts/build_sample_masters_xml.py
# Then move the full file BACK out before committing:
mv rgi_migration/tests/fixtures/sample_cacspu_masters.xml \
   ~/tally-exports/cacspu_masters.xml
git diff rgi_migration/tests/fixtures/sample_cacspu_masters_sample.xml
```
